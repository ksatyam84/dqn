...
class WumpusWorldEnvironment(gym.Env):
    ...
    def step(self, action):
        ...
        self.take_action(action)
        self.agent_pos = np.clip(self.agent_pos, a_min=[0, 0], a_max=[self.environment_width - 1, self.environment_height - 1])
        observation = self.return_observation()
        self.timesteps += 1
        ...
        elif any(np.array_equal(self.agent_pos, pit) for pit in self.pit_pos) or np.array_equal(self.agent_pos, self.wumpus_pos):
            reward = -100
            terminated = True
        else:
            reward = -1
